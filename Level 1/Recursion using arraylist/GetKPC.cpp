...
vector<string> words = {".,","abc","def","ghi","jkl","mno","pqrs","tu","vwx","yz"};
...
vector<string> recurse(string input){
    ...
    vector<string> restwords = recurse(rest);  

    string firstSize = words[firstLetter - '0'];

    vector<string> newans;

    for(int i=0;i<firstSize.size();i++){
        for(int j=0;j<restwords.size();j++){
            newans.push_back(firstSize[i]+restwords[j]+"");
        }
    }
    return newans;
